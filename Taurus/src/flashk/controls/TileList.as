package flashk.controls
{
	...
	public class TileList extends Tile
	{
		 ...
		 * 添加数据 
		 * @param v
		 * @param args i横坐标序号   j纵坐标序号
		 * 
		 */		
		public function addData(v:*,... args):void
		{
			var index:uint;
			var i:uint=0;
			var j:uint=0;
			...
			if (type == UIConst.HORIZONTAL)
				...
				index = j * columnCount + i;
			if (!v)
				v = ref ? ref.newInstance() : new Shape();
			addContent(i,j,v);
			data.splice(index,0,1);
			render();
		}
		private function addContent(i:int,j:int,item:DisplayObject):void
		{
			var ary:Array=[];
			var id:int;
			var jd:int;
			...
			var index:uint=0;
			var len:uint=ary.length;
			var row:uint=rowCount;
			if(len>rowCount*columnCount)
			{
				row++;
			}
			for (jd = 0;jd < row;jd++)
